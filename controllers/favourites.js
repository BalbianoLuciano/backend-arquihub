...
const updateFavourites = async (req, res) => {
  ...
  const favouriteRepeat = user.favourites.find( e=>  e.equals(user_id))?true:false
...
const deleteFavourites = async (req, res) => {
  try {
    ...
    if(!id||!user_id)return res.status(404).send({err:"requiered id of user and post to delete of favourites"})
    await usersModel.findOne({_id:user_id});
    await postModel.findOne({_id:id});
    await usersModel.updateOne({_id:user_id},
      { $pull: { favourites_post: id } },
      { new: true, useFindAndModify: false }
    );
